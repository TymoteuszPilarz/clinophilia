...
class OBSController:
    ...
    def start_recording(self):
        ...
        while True:
            try:
                self.obs_client = obs.ReqClient(host=self._host, port=self._port, password=self._password, timeout=5)
                break
        ...
        self.obs_client.create_input(self.scene_name, self.scene_name + '_dummy_source', 'screen_capture', {}, False)
        windows = self.obs_client.get_input_properties_list_property_items(self.scene_name + '_dummy_source', 'window')
        ...
        self.obs_client.create_input(self.scene_name, self.scene_name + '_source', 'screen_capture', {'application': 'com.microsoft.teams2', 'show_hidden_windows': True, 'type': 2, 'window': teams_window_id}, True)
